...
function generateText(x: number, y: number, text: string, align = 'start', transform = '') {
	return `
		<text
			x="${x}"
			y="${y}"
...
function generateBar(y: number, max: number, index: number, label: string) {
...
export function generateBarGraph(points: { y: number; label: string }[], yLegend: string) {
